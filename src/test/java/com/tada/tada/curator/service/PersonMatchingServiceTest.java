...
class PersonMatchingServiceTest {
	...
	@BeforeEach
	void setUp() {
		memoryPersonRepository = Mockito.mock(MemoryPersonRepository.class);
		personAliasRepository = Mockito.mock(PersonAliasRepository.class);
	...
	void 기존_인물_한명과_정확히_일치하면_exact를_반환한다() {
		...
		PersonMatchResult result =
				personMatchingService.match(userId, "한영이와");
	...
	void 같은_이름의_인물이_두명이면_ambiguous를_반환한다() {
		...
		PersonMatchResult result =
				personMatchingService.match(userId, "민수와");
	...
	void memoryPerson과_alias가_같은_personId면_한명으로_처리한다() {
		...
		PersonMatchResult result =
				personMatchingService.match(userId, "민수");
	...
	@Test
	void 정확한_인물이_없고_비슷한_이름이_있으면_similar를_반환한다() {
		UUID userId = UUID.randomUUID();
		UUID personId = UUID.randomUUID();
		...
		PersonMatchResult result =
				personMatchingService.match(userId, "민서");
	...
	void 정확하거나_비슷한_인물이_없으면_new를_반환한다() {
		...
		PersonMatchResult result =
				personMatchingService.match(userId, "철웅");
	...
	private MemoryPerson createMemoryPerson(
		...
		MemoryPerson person = newInstance(MemoryPerson.class);
		
		setField(person, "id", id);
		setField(person, "userId", userId);
		setField(person, "displayName", displayName);
		setField(person, "createdAt", LocalDateTime.now());
	...
	private PersonAlias createPersonAlias(
		...
		PersonAlias alias = newInstance(PersonAlias.class);
		
		setField(alias, "id", id);
		setField(alias, "personId", personId);
		setField(alias, "ownerUserId", ownerUserId);
		setField(alias, "aliasText", aliasText);
		setField(alias, "normalizedText", normalizedText);
